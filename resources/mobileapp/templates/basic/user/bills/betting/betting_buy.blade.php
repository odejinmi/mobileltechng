...
@section('panel')
      ...
      <div class="swiper banner">
        <div class="swiper-wrapper">
          <div class="swiper-slide">
            <a href="#">
              <img class="img-fluid banner-img" src="{{ asset($activeTemplateTrue . 'mobile/images/banner/sportsbettingposter.jpg')}}" alt="banner1" />
            </a>
          </div>

          <div class="swiper-slide">
            <a href="#">
              <img class="img-fluid banner-img" src="{{ asset($activeTemplateTrue . 'mobile/images/banner/sportsbettingposter.jpg')}}" alt="banner2" />
            </a>
          </div>
        </div>
      </div>
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    @foreach ($networks as $data)
                                        <li  onclick="networkprovider('{{ strToUpper($data['provider']) }}','{{$general->cur_sym}} {{ @number_format($data['minAmount']['amount'],2) }}','{{$general->cur_sym}} {{ @number_format($data['maxAmount']['amount'],2) }}')">
                                            <a class="dropdown-item"><img class="img-fluid currency-icon" src="{{ $data['providerLogoUrl'] }}"
                                                    alt="provider" />{{ strToUpper($data['provider'])}}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                       ...
                       @endpush
                        ...
                        <script>
                            function verifywallet(e) {
                                ...
                                fetch("{{ route('user.betting.wallet.verify') }}", requestOptions)
  ...
  <section class="pay-money section-b-space" id="step2">
    <div class="custom-container">
        <form class="auth-form" action="{{ route('user.crypto.sell.confirm.manual') }}" method="POST" enctype="multipart/form-data">
            @csrf                                          
         <div class="custom-container">
             <div class="profile-pic">
             <img class="img-fluid img" src="{{ asset('assets/assets/dist/images/backgrounds/2.png') }}" alt="p3" />
             </div>
             <h3 class="person-name">@lang('Please send coin to the wallet address below')</h3>
...
<a href="{{ route('user.betting.history') }}" class="back-btn">
