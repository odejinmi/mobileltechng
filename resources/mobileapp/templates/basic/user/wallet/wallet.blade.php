...
<section>
      ...
      <div class="crypto-wallet-box">
        <div class="card-details">
          <div class="d-block w-75">
            ...
            <h2 class="mt-2">{{ $wallet->balance }}<small>{{ strToUpper($coin->symbol) }}</small></h2>
          </div>
          <div class="price-difference">
            <h6 >
                <img class="img-fluid icon" width="40" src="{{ url('/') }}/assets/images/coins/{{@$coin->image}}" alt="bitcoins" />
  ...
  <section>
    <div class="custom-container">
      <div class="chart mb-3">
        ...
        <div style="width: 250spx; max-width:100% height:220px;  overflow:hidden; box-sizing: border-box; border: 1px solid #56667F; border-radius: 4px; text-align: right; line-height:14px; block-size:220px; font-size: 12px; font-feature-settings: normal; text-size-adjust: 100%; box-shadow: inset 0 -20px 0 0 #56667F;padding:1px;padding: 0px; margin: 0px;"><div style="height:200px; padding:0px; margin:0px; max-width:100%"><iframe src="https://widget.coinlib.io/widget?type=single_v2&theme=light&coin_id={{ $coin->code }}&pref_coin_id=1505" width="350"  height="196px" scrolling="auto" marginwidth="0" marginheight="0" frameborder="0" border="0" style="pointer-events: none; max-width:100%; border:0;margin:0;padding:0;line-height:14px;"></iframe></div></div>
    </div>
      </div>
    </div>
  </section>
  ...
  <section class="categories-section section-b-space">
    <div class="custom-container">
      <ul class="categories-list">
        ...
        <li data-bs-toggle="modal" data-bs-target="#sell-coin">
          <a href="#">
            <div class="categories-box">
              <i class="iconsax categories-icon" data-icon="repeat"></i>
            </div>
            <h5 class="mt-2 text-center">Sell All</h5>
          </a>
        </li>
  ...
  <section>
    <div class="custom-container">
      <div class="title">
        ...
        <a href="{{ route('user.crypto.wallet.trx', $wallet->address) }}">See all</a>
      </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h2 class="modal-title">@lang('Receive') {{ $coin->name }}</h2>
        </div>
          ...
          <div class="form-group"  onclick="myFunction()" >
            <div class="form-group mt-1">
                <div class="form-inputs" id="">
                    <input type="text"id="walletaddress" value="{{ $wallet->address }}" readonly class="form-control reasons">
                </div>
            </div> 
            </section>
  ...
  <div class="modal add-money-modal fade" id="swap-coin" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h2 class="modal-title">@lang('Swap') {{ $coin->name }}</h2>
        </div>
        <div class="modal-body">
            <section class="pay-money section-b-space">
          ...
          <a href="#" onclick="swapcoin()" id="swapbutton" disabled  class="btn theme-btn successfully w-100"> @lang('Swap') {{ $coin->name }}</a>
        </div>
        <button type="button" class="btn close-btn" data-bs-dismiss="modal">
          <i class="icon" data-feather="x"></i>
        </button>
...
<!-- successful modal end -->
 ...
 <form method="POST" action="{{ route('user.crypto.sellall',encrypt($coin->id)) }}" enctype="multipart/form-data">
            @csrf
  <!-- Send Coin modal starts -->
  <div class="modal add-money-modal fade" id="sell-coin" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h2 class="modal-title">@lang('Sell All') {{ $coin->name }}</h2>
        </div>
        <div class="modal-body">
            ...
            <div class="row mb-10">
               ...
               <div class="text-center pt-15">
                  <button class="btn theme-btn successfully w-100" type="submit">
                ...
                fetch("{{ route('user.crypto.exchange', encrypt($coin->id)) }}", requestOptions)
                    ...
                    .then(result => {
                        ...
                        else
                        {
                            ...
                            document.getElementById("swapbutton").disabled = false;
                            $("#getswaprate").html(
                                `<center><a class="badge bg-info text-white" href="javascript:void(0);">${getvalue}{{$general->cur_text}}</a></center>`
                            );
                        }
                    })
                    .catch(error => {
...
<a href="{{ route('user.crypto.index') }}" class="back-btn">
    <i class="icon" data-feather="x"></i>
  </a>
@endpush
